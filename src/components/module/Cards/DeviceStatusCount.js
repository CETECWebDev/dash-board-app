...
function DeviceStatusCount({ devices, activeDeviceCount, setActiveDeviceCount }) {
  const totalDevices = Array.isArray(devices) ? devices.length : 0;
  const activeDevices = Array.isArray(devices) ? devices.filter(device => device.active === true).length : 0;
